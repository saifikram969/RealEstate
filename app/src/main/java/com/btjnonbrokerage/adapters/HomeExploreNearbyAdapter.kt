...
import com.btjnonbrokerage.MVVM.APIModel.NearEstate.Property
...
class HomeExploreNearbyAdapter(val onClick : (Property) -> Unit, val clickOnLike : (Property)->  Unit, val clickOnUnLike : (Property)-> Unit) : ListAdapter<Property, HomeExploreNearbyAdapter.MyViewHolder>(Diff) {
        ...
        fun bind(item: Property) {
            ...
            if(item.images.isNotEmpty()) {
                Glide.with(binding.root.context).load(item.images[0])
                    .placeholder(R.drawable.home_ph).into(binding.ivPropertyImage)
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return  MyViewHolder(ItemPropertyLayoutBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
